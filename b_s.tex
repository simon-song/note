...
Here we study the Monte Carlo method, especially the impact of GPU computing, by
comparing the result and computing time using CPU and GPU.

We run the experiments on a Linux system witha a 16-core Intel i7 CPU (2.50GHz),
...
About $70\%$ of the paths would result in a knockout event, of the remaining
paths, about $12\%$ would result in a knockin event, and the rest $13\%$ would
result in no hit.

Next we compare the pricing results and computing time of using a single CPU,
and that of using a single GPU, with several different choices of paths. 
...
$1,000,000$ paths, and $670$-fold when using $10,000,000$ paths.
...
computing time of autocallable pricing.
